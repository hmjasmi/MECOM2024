function BER1th = func_BERth_U1_Gray(b,a,SNR_dB,channel,channel_factor)
...
d_m = 2.*m;
for k = 1:log2(sqrt(M(1)))
    i = [0:((2- 2^(1-k))*sqrt(M(1))-1)].';
    d_i = 2.*i - sqrt(M(1))*(2-2^(1-k)) + 1;
    temp = nchoosek_sVer2(2,2^(k-1));
    temp_ = temp(floor(2^k.*i./2./sqrt(M(1)))+1);
    temp_ = reshape(temp_,[length(temp_),1]);
    c{k} = 2./sqrt(prod(M)).*(-1).^(-(m+1)*log10(k)/log10(max([2,k]))+(m+1)).*(sqrt(M(2))-m).*...
           (-1).^floor(2^k*i/sqrt(M(1))-1*log10(k)/log10(max([2,k]))*(2.^(k-1).*i/sqrt(M(1))-1/2)).*...
           temp_;
    c{k} = c{k}(:);
